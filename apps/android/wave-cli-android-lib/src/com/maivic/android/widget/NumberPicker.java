...
/**
 * NumberPicker widget.
 * This is implementation of specification: https://github.com/bic/wave-cli-app/issues/2
 */
public class NumberPicker extends RelativeLayout implements LayoutSetupHelper.ActionButtonClickCallback{
	...
	public void setGroup_size(int group_size) {
		if (group_size == 0)
			throw new IllegalStateException("The group_size value must be more 0");
	...
	private LayoutSetupHelper getDefaultLayoutHelper(){
		SimpleLayoutSetupHelper layoutHelper = new SimpleLayoutSetupHelper(getContext());
...
//	}
	 ...
	 *            notify listener
	 */
	public synchronized void setValue(BigDecimal newValue, boolean fireNotification){
...
//		setupActionButtons();
	...
	public void onActionButtonClick(View buttonView, BigDecimal stepValue) {
		if(stepValue != null){
			BigDecimal nextValue = Utils.next(value, stepValue, reference, min_value, max_value);
...
//			doValueUpdateAction(value, stepValue);				
	...
	void enableDisableActionButtons(boolean applyForIncrementButtons, boolean enabled){
		...
		for(View ab: actionButtons){
			BigDecimal deltaValue = mLayoutSetupHelper.getStepValueForActoinButton(ab);
			if(deltaValue != null){
				if(deltaValue.signum() < 0 && !applyForIncrementButtons){
					// update decrement button state 
					mLayoutSetupHelper.setButtonEnabledState(ab, buttonIndex, enabled);
...
//				&& (value == 0 || (value == 1 && min_value == 0 && max_value == 1))) {				
				&& (value.equals(zero) || (value.equals(one) && min_value.equals(zero) && max_value.equals(one)))) {
			
			// show zero checkbox button
			mLayoutSetupHelper.showZeroCheckbox();			
			mLayoutSetupHelper.setLabelValue(label, "");
			mLayoutSetupHelper.setZeroCheckboxChecked(value.compareTo(one) == 0);
			...
			if(value.equals(one) || value.equals(minusOne)){
				...
				if(label_plural != null)
					mLayoutSetupHelper.setLabelValue(label_plural, String.valueOf(value));
				else 
					mLayoutSetupHelper.setLabelValue(label, String.valueOf(value));
	...
	public static interface OnValueChangedListener{
		public void onValueChanged(NumberPicker widget, BigDecimal oldValue, BigDecimal newValue);
...
//				
//				buttonsContainer
//						.setOrientation(mNumberPicker.getAlign() == AlignType.HORIZONTAL ? LinearLayout.VERTICAL
