...
public class MultipleButtonsSimpleLayoutNumberPicker extends SimpleLayoutSetupHelper{

		public MultipleButtonsSimpleLayoutNumberPicker(Context context) {
			super(context);
		}
		...
		public void setLabelValue(String label, String value) {
			...
			txtValueView.setText(convertValueToTime(value));
		}
		
		private String convertValueToTime(String value){
			String cValue = null;
			int Mins = Integer.parseInt(value);
			int hours = Mins / 60;
			int min = Mins % 60;
			if (min == 0){
				cValue = Integer.toString(hours) + ":" + Integer.toString(min)+"0";
			} else if (min == 5){
				cValue = Integer.toString(hours) + ":0" + Integer.toString(min);
			}else {
				cValue = Integer.toString(hours) + ":" + Integer.toString(min);
			}
